    ...
    private void saveTasks() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH))) {
            for (Task task : tasks) {
                ...
                writer.write(" ");
                writer.write(task.getDueDate());//also write into file each task's due date
                writer.newLine();
            }
        }
    ...
    private void loadTasks() {
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_PATH))) {
            ...
            while ((taskDescription = reader.readLine()) != null) {
                String dueDateString = taskDescription.substring(taskDescription.length()-11);
                LocalTime timeNow = LocalTime.now();
                LocalTime dueDate = LocalTime.parse(dueDateString);
                if(dueDate.isAfter(timeNow)) {
                    tasks.add(new Task(taskDescription,dueDateString));
                }
            }
        }
        // If exception occurs, print stack trace error
